...
This archive is the source of truth for turning the design into production code. Start from `00-login.html`, then preserve the visual system, responsive behavior, and interactions found in the exported files.
...
- A single self-contained `00-login.html` is acceptable only when the export truly contains one user-facing screen and its CSS/JS are structured enough to extract tokens, components, states, and behavior.
...
1. Open `00-login.html` and `DESIGN-MANIFEST.json`; identify every screen file, launcher/overview file, app module, and interaction before coding.
...
1. Inspect `00-login.html` and `DESIGN-MANIFEST.json` first and identify reusable components before coding.
